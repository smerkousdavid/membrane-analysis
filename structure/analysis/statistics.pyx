...
from structure.analysis.statistics cimport StatsResults, compute_stats
...
cdef class Statistics(object):
    ...
    def get_count(self):
        if self.stats.is_nan(self.stats.count):
            return math.nan
    ...
    @staticmethod
    def get_header_data():
        return ['Count', 'Sum', 'Mean', 'Variance', 'STD', 'CV', 'Min', 'Max', 'Range', 'Q1', 'Median', 'Q3', 'IQR']

    def get_row_data(self):
        return [self.get_count(), self.get_sum(), self.get_mean(), self.get_variance(), self.get_STD(), self.get_CV(), self.get_min(), self.get_max(), self.get_range(), self.get_Q1(), self.get_median(), self.get_Q3(), self.get_IQR()]
