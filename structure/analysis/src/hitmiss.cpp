...
namespace hitmiss {
    ...
    // @TODO try to support matrices that are greater than a 3x3 for now match_size must be a value of 3 (for edge scanning)
    std::vector<cmp::location> convolve_match_image(const uint8_t* image, const uint8_t* matches, const uint32_t rows, const uint32_t cols, const uint32_t num_matches, const uint32_t match_size, const uint8_t scan_edge) {
        ...
        if (match_size == 3 && scan_edge == (uint8_t) 1) {
            // scan top row
            cur_row = 0;
        ...
        const int matrix_offset = ((match_size - 1) / 2);
        
        for (int row = matrix_offset; row < rows - matrix_offset; row++) {
            for (int col = matrix_offset; col < cols - matrix_offset; col++) {
