...
def elem_angle(point1, point2):
    return np.arctan2(point1, point2)  # swap possibly later
...
def simplify_membrane_edge(layers):
    ...
    for line in revised_lines:
        ...
        for div in divided:
            ...
            # let's create the two angles of travel
            ...
            # for the extreme cases of large slope let's subsample
            if slope > height:  # slope shouldn't be greater than image height
                subsampling = 1.0 / height
            elif abs(slope) > 1.0:  # let's interpolate between 1 (x-y) to (x-heightish)
                subsampling = abs(1.0 / slope)

            # to be safe let's double image ratio sub-sampling
            subsampling *= (1.0/4.0)

            # do extra steps for large slopes due to subsampling
            if subsampling != 1.0:
                steps = width * m.ceil(abs(slope) if abs(slope) < height else height) * height
            else:
                steps = width * height
            ...
            for i in range(steps):
                back_pt = float(pt1[0]) - (float(i) * subsampling)
                fwd_pt = float(pt1[0]) + (float(i) * subsampling)
                back_y = round(eq(back_pt))
                fwd_y = round(eq(fwd_pt))

                if backward_continue:
                    try:
                        if back_pt > 0 and eq(back_pt) > 0 and layers['membrane'][int(round(back_y)), int(round(back_pt))] > 1:
                            back_count += 1
                            back_line.append((int(round(back_pt)), int(back_y)))
                        else:
                ...
                if forward_continue:
                    try:
                        if fwd_pt < width and eq(back_pt) < height and layers['membrane'][int(round(fwd_y)), int(round(fwd_pt))] > 1:
                            forward_count += 1
                            forward_line.append((int(round(fwd_pt)), int(fwd_y)))
                        else:
